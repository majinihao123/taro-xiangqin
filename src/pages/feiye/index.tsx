    ...
    <View className='modal-overlay' onClick={onCancel}>
      <View className='modal-card' style={{ boxShadow: `0 40rpx 80rpx rgba(0,0,0,0.28), 8rpx 8rpx 0 ${pick.color}` }} onClick={e => e.stopPropagation()}>
        ...
        <View className='modal-card__seal-area'>
          ...
          {stamping ? (
            <View
              className='modal-card__seal-stamp'
              style={{ color: pick.color, borderColor: pick.color }}
            >
              <Text className='modal-card__seal-stamp-text' style={{ color: pick.color }}>
                {pick.zh}
              </Text>
            </View>
          ) : (
        ...
        <View className='modal-card__btns'>
          <View
            ...
            onClick={stamping ? undefined : onCancel}
          >
...
export default function FeiyePage() {
  ...
  function handleSubmit(key: IdentityKey) {
    try { Taro.setStorageSync('xq_identity', key) } catch {}
    setConfirming(false)
  ...
  return (
    <View className='feiye-page'>
        ...
        {OPTIONS.map((o, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <Text
                  className='option-card__circle-text'
                  style={{ color: active ? '#FBF5E4' : o.color }}
                >
                  {active ? '✓' : o.icon}
                </Text>
              </View>
              ...
              <View className='option-card__info'>
                <View className='option-card__title-row'>
                  <Text className='option-card__zh' style={{ color: active ? o.color : '#2B2620' }}>
                    {o.zh}
                  </Text>
      ...
      <View className='submit-area'>
        <View
          className={`submit-area__btn${pick ? ' submit-area__btn--active' : ' submit-area__btn--inactive'}`}
          onClick={pick ? () => setConfirming(true) : undefined}
        >
          <Text className={`submit-area__btn-text${pick ? ' submit-area__btn-text--active' : ' submit-area__btn-text--inactive'}`}>
            {pick ? '盖章 · 开本' : '请先选择一项'}
          </Text>
        </View>
